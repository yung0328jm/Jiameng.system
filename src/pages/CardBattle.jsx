...
export default function CardBattle({ playerDeck, enemyDeck, playerAccount, onExit, playerCardBackUrl, enemyCardBackUrl, roomId, mySide }) {
  ...
  const renderViewDetailModal = () => {
    ...
    if (viewDetail.type === 'hero') {
      ...
      const container = (isLandscapeFullscreen && battleContainerRef.current) ? battleContainerRef.current : document.body
      return typeof document !== 'undefined' && container ? createPortal(modalContent, container) : modalContent
    }
    ...
    if (viewDetail.type === 'field') {
      ...
      const containerField = (isLandscapeFullscreen && battleContainerRef.current) ? battleContainerRef.current : document.body
      return typeof document !== 'undefined' && containerField ? createPortal(modalContent, containerField) : modalContent
    }
    return null
  }
  ...
  const renderHandDetailModal = () => {
    ...
    const containerHand = (isLandscapeFullscreen && battleContainerRef.current) ? battleContainerRef.current : document.body
    return typeof document !== 'undefined' && containerHand ? createPortal(modalContent, containerHand) : modalContent
  }
