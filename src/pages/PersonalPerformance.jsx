...
import { removeWorkDetailLineFromSchedule } from '../utils/scheduleWorkDetailRemoval'
...
function PersonalPerformance() {
  ...
  const calculatePerformance = (userName) => {
    ...
    schedules.forEach((schedule) => {
      ...
      // 藍標（4/1 起每人填寫）與黃標住宿：接駁＋異動人員依人計分；藍標 2026/3 止仍走預排 workItems，避免與舊預排雙計分
    ...
    // 當月請假天數（已核准且申請人為當前查看用戶）
    const leaveApps = getLeaveApplications().filter((la) => (la.status || '') === 'approved')
    let leaveDays = 0
    leaveApps.forEach((la) => {
      const matchUser = String(la.userId || '').trim() === String(userName || '').trim() ||
        namesToMatch.some((n) => String(la.userName || '').trim() === n || String(la.userId || '').trim() === n)
      if (!matchUser) return
      const laStart = String(la.startDate || '').trim()
      const laEnd = String(la.endDate || '').trim()
      if (!laStart || !laEnd) return
      const overlapStart = laStart < startDate ? startDate : laStart
      const overlapEnd = laEnd > endDate ? endDate : laEnd
  ...
  const isBusinessTripSchedule = (schedule) => {
    ...
    return (
      tag === 'yellow' ||
      /^出差(\s|[-—])/u.test(siteName) ||
      /^住宿(\s|[-—])/u.test(siteName) ||
      siteName === '出差' ||
      siteName === '住宿'
    )
  }
  ...
  const handlePreviewImport = async () => {
    ...
    const leaveKeywords = ['請假', '特休', '補休', '公假', '病假', '事假', '喪假', '婚假', '產假', '育嬰', '出差', '住宿']
    const isLeaveMark = (s) => {
    ...
    const preview = parsed.map((row, index) => {
      ...
      if (dateTimeField) {
        ...
        if (!rawTimeStr) {
          ...
          // 若行事曆該日為住宿（黃標／舊稱出差）排程：顯示為住宿，不算未打卡
  ...
  return (
    <div ref={performancePdfRef} className="bg-charcoal rounded-lg p-4 sm:p-6 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        ...
        {/* 出勤記錄詳情 */}
        {showLateRecords && (() => {
          // 從導入預覽中計算未打卡記錄（排除週末、住宿排程與國定假日）
          // 邏輯：1. 將"缺少打卡紀錄"視為未打卡 2. 匯入檔案後，沒有記錄的日期一律視為未打卡 3. 但要排除週六、週日、住宿排程與國定假日
          ...
          if (importPreview && importPreview.length > 0) {
            ...
            // 1. 處理"缺少打卡時間"的記錄（視為未打卡，但排除週末、住宿排程與國定假日）
            importPreview.forEach(item => {
                        ...
                        <tr key={record.id || index} className="border-b border-gray-700 hover:bg-gray-750">
                          ...
                          <td className="px-2 py-1.5 text-center">
                            {isNationalHoliday ? (
                              ...
                              <span className="text-blue-400 font-semibold text-[10px] sm:text-xs">住宿</span>
                            ) : isLeave ? (
                ...
                <div className="bg-gray-900 rounded-lg p-4 border border-gray-700">
                          ...
                          <tr key={idx} className="border-b border-gray-700">
                            ...
                            <td className="px-3 py-2 text-center">
                              {item.holiday ? (
                                ...
                                <span className="text-blue-400 font-semibold">住宿</span>
                              ) : item.error ? (
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <div className="text-sm text-gray-400">
                      ...
                      住宿: <span className="text-blue-400">{importPreview.filter(p => p.trip).length}</span> 筆，
