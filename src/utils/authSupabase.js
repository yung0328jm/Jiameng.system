...
/**
 * 離職者帳號／顯示名（僅兩欄，供出工回報等過濾勾選名單）。
 * 須在 Supabase 執行 docs/supabase-get-resigned-identifiers.sql 建立 RPC；未建立時回傳空陣列。
 */
export async function getResignedProfileIdentifiers() {
  const sb = getSupabaseClient()
  if (!sb) return []
  const { data, error } = await sb.rpc('get_resigned_profile_identifiers')
  if (error) {
    console.warn('getResignedProfileIdentifiers', error)
    return []
  }
  return Array.isArray(data) ? data : []
}
