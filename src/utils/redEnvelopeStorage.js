...
const defaultConfig = () => ({
  itemId: '',           // 紅包卡道具 ID（請先在道具管理新增）
  maxPerUser: 0,        // 每用戶最多可領總數（0 = 關閉活動）
...
export const grabRedEnvelope = (account) => {
  ...
  if (!config.itemId) return { success: false, message: '管理員尚未設定紅包卡道具' }
  ...
  if (claimed >= config.maxPerUser) {
    return { success: false, message: `已達本活動上限（${config.maxPerUser}），下次請早` }
  }

  const remaining = config.maxPerUser - claimed
  const minQty = Math.min(config.minQtyPerGrab, remaining)
  const maxQty = Math.min(config.maxQtyPerGrab, remaining)
  const qty = minQty >= maxQty ? minQty : minQty + Math.floor(Math.random() * (maxQty - minQty + 1))
  if (qty <= 0) return { success: false, message: '已達上限' }

  const res = addItemToInventory(acc, config.itemId, qty)
  if (!res.success) return { success: false, message: res.message || '發放失敗' }

  const claims = getRedEnvelopeClaims()
  claims[acc] = (Number(claims[acc]) || 0) + qty
  persistClaims(claims)

  return { success: true, message: `恭喜獲得 ${qty} 個紅包卡！`, quantity: qty }
}
